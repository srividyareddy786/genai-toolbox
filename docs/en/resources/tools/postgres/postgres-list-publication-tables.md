...
`postgres-list-publication-tables` lists detailed information as JSON for publication tables. The tool takes the following input parameters:

- `table_name` (optional): A text to filter results by table name. The input is used within a LIKE clause. Default: `""`
- `publication_name` (optional): A text to filter results by publication name. The input is used within a LIKE clause. Default: `""`
