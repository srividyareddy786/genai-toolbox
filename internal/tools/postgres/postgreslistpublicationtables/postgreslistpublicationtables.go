...
const listPublicationTablesStatement = `
	...
	WHERE
		($1::text IS NULL OR table_name LIKE '%' || $1::text || '%')
		AND ($2::text IS NULL OR publication_name LIKE '%' || $2::text || '%')
		AND ($3::text IS NULL OR schema_name LIKE '%' || $3::text || '%')
	ORDER BY
...
func (cfg Config) Initialize(srcs map[string]sources.Source) (tools.Tool, error) {
	...
	allParameters := tools.Parameters{
		tools.NewStringParameterWithDefault("table_name", "", "Optional: A specific table name to search for."),
		tools.NewStringParameterWithDefault("publication_name", "", "Optional: A specific publication name to search for."),
		tools.NewStringParameterWithDefault("schema_name", "", "Optional: A specific schema name to search for."),
