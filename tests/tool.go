...
func RunPostgresListPublicationTablesTest(t *testing.T, ctx context.Context, pool *pgxpool.Pool) {
	tableName := "pub_table_" + strings.ReplaceAll(uuid.New().String(), "-", "")
	pubName := "pub_" + strings.ReplaceAll(uuid.New().String(), "-", "")

	cleanup := setupPostgresPublicationTable(t, ctx, pool, tableName, pubName)
	defer cleanup(t)
	...
	invokeTcs := []struct {
	...
	}{
		{
			...
			requestBody:    bytes.NewBufferString(fmt.Sprintf(`{"publication_name": "%s"}`, pubName)),
			wantStatusCode: http.StatusOK,
			want:           []map[string]any{wantTable},
		},
		{
			name:           "filter by table_name",
			requestBody:    bytes.NewBufferString(fmt.Sprintf(`{"table_name": "%s"}`, tableName)),
			wantStatusCode: http.StatusOK,
			want:           []map[string]any{wantTable},
		},
		{
			name:           "filter by schema_name and table_name",
			requestBody:    bytes.NewBufferString(fmt.Sprintf(`{"schema_name": "public", "table_name": "%s"}`, tableName)),
			wantStatusCode: http.StatusOK,
			want:           []map[string]any{wantTable},
		},
		{
